...
let { verify } = require('../routes/middleware/authentication');	var controller = require('../controllers/api/access.controller');
	...
	app.get('/api/access/findby/id_user/:id_user', verify('find by access id_user'), controller.findById_user);
	app.get('/api/access/findby/endpoint/:endpoint', verify('find by access endpoint'), controller.findByEndpoint);
